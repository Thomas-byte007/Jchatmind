 ...
 * ?????????? -- ??????????????????
 * ??(Session) = ?????????,??????,????Agent
 * getChatSessionsByAgentId():????Agent??????
 */
public interface ChatSessionFacadeService {
    ...
    CreateChatSessionResponse createChatSession(CreateChatSessionRequest r);
    void deleteChatSession(String chatSessionId);
    void updateChatSession(String chatSessionId, UpdateChatSessionRequest r);
}