...
public class MarkdownParserServiceImpl implements MarkdownParserService {
    ...
    public List<MarkdownSection> parseMarkdown(InputStream inputStream) {
        try {
            ...
            log.info("?? Markdown ??,??? {} ???", sections.size());
            return sections;
        } catch (Exception e) {
            log.error("?? Markdown ??", e);
            throw new RuntimeException("?? Markdown ??: " + e.getMessage(), e);
        }
    }
    ...
    private String extractNodeContent(Node node) {
        ...
        if (node instanceof TableBlock) return extractTableMarkdown(node);
        return extractPlainText(node);
    }

    // ???Markdown????????????????
    private String extractTableMarkdown(Node tableNode) {
        if (originalMarkdownContent == null) return extractPlainText(tableNode);
        try {
            ...
            log.warn("???? Markdown ??,??????: {}", e.getMessage());
            return extractPlainText(tableNode);
        }
    }
    ...
    private void extractTextRecursive(Node node, StringBuilder text) {
        if (node == null) return;
        if (node instanceof Heading) return;  // ????extractSections?????
